...
from rag_engine import AskSkillveri, load_knowledge_base, TFIDFRetriever
...
kb_path = os.path.join(os.path.dirname(__file__), 'domain_knowledge.json')
...
if page == "💬 Chat":
    ...
    with chat_col:
        ...
        try:
            ...
            # Show retriever type
            if rag.retrieval_method == "vector_db":
                st.success(f"🗄️ **Vector DB Active** — ChromaDB + Sentence Transformers ({len(rag.documents)} docs)")
            else:
                st.info(f"📝 Using TF-IDF retrieval ({len(rag.documents)} docs). Install `chromadb` and `sentence-transformers` for vector search.")
